...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('C4T-RM/SMD/smd_gap10/'),
...
trainData = np.load(f'C4T-RM/SMD/smd_gap10/smd_gap10_Train.npy',
                    allow_pickle=False)
testData = np.load(f'C4T-RM/SMD/smd_gap10/smd_gap10_Test.npy',
                   allow_pickle=False)
valData = np.load(f'C4T-RM/SMD/smd_gap10/smd_gap10_val.npy',
                  allow_pickle=False)
test_ground_truth = np.load(
    f'C4T-RM/SMD/smd_gap10/smd_gap10_test_ground_truth.npy',
    allow_pickle=False)
val_ground_truth = np.load(
    f'C4T-RM/SMD/smd_gap10/smd_gap10_val_ground_truth.npy',
    allow_pickle=False)
...
valData=processed_data[16470:22470].astype(np.float32).copy()
testData=processed_data[22470:].astype(np.float32).copy()
samples_list = list()
...
class Generator_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm0 = nn.LSTM(input_size=38, hidden_size=64, num_layers=1, batch_first=True)

        self.linear = nn.Conv1d(in_channels=30, out_channels=30, kernel_size=27, stride=1)
        # embedding
        ...
        self.lstm = nn.LSTM(input_size=38, hidden_size=128, num_layers=1, batch_first=True)

        self.main = nn.Sequential(


            nn.Conv1d(in_channels=30, out_channels=30, kernel_size=22, stride=2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=30, out_channels=30, kernel_size=12, stride=1),
            nn.BatchNorm1d(opt.window_size, eps=0.001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=30, out_channels=30, kernel_size=6, stride=1),
            nn.Sigmoid()
        )
