...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('C4T-RM/EPIC/epic_gap10/'),
...
trainData = np.load(
    f'C4T-RM/EPIC/epic_gap10/epic_gap10_Train.npy',
    allow_pickle=False)
testData = np.load(
    f'C4T-RM/EPIC/epic_gap10/epic_gap10_Test.npy',
    allow_pickle=False)
valData = np.load(f'C4T-RM/EPIC/epic_gap10/epic_gap10_val.npy',
                  allow_pickle=False)
test_ground_truth = np.load(
    f'C4T-RM/EPIC/epic_gap10/epic_gap10_test_ground_truth.npy',
    allow_pickle=False)
val_ground_truth = np.load(
    f'C4T-RM/EPIC/epic_gap10/epic_gap10_val_ground_truth.npy',
    allow_pickle=False)
...
valData=processed_data[480:660].astype(np.float32).copy()
testData=processed_data[660:].astype(np.float32).copy()
samples_list = list()
...
class Generator_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm0 = nn.LSTM(input_size=291, hidden_size=512, num_layers=1, batch_first=True)
        self.linear = nn.Linear(512, 291)
        # embedding
        ...
        self.lstm = nn.LSTM(input_size=291, hidden_size=256, num_layers=1, batch_first=True)
...
class Encoder_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=291, hidden_size=256, num_layers=2, batch_first=True)
        self.main = nn.Sequential(
...
class Discriminator_i(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.LSTM(input_size=291, hidden_size=512, num_layers=1, batch_first=True)
        )
