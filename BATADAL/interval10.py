...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('C4T-RM/BATADAL/batadal_interval10/'),
...
trainData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_interval10_Train.npy',
    allow_pickle=False)
testData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_interval10_Test.npy',
    allow_pickle=False)
valData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_interval10_val.npy',
    allow_pickle=False)
test_ground_truth = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_interval10_test_ground_truth.npy',
    allow_pickle=False)
val_ground_truth = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_interval10_val_ground_truth.npy',
    allow_pickle=False)
...
class Generator_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.time_series_layer = nn.Conv2d(
            in_channels=3, out_channels=1, kernel_size=(11, 11), stride=(1, 1), padding=(5, 5), bias=True)
        self.lstm0 = nn.LSTM(input_size=36, hidden_size=64, num_layers=1, batch_first=True)
        self.linear = nn.Linear(64, 36)

        # embedding
        self.encode = nn.Sequential(
            nn.Conv2d(in_channels=91, out_channels=32, kernel_size=(11, 11), stride=(1, 1), padding=(5, 5), bias=True),
            nn.Conv2d(in_channels=32, out_channels=1, kernel_size=(11, 11), stride=(1, 1), padding=(5, 5), bias=True)
        )

        self.lstm = nn.LSTM(input_size=36, hidden_size=128, num_layers=1, batch_first=True)
