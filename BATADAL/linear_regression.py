...
import torch
import h5py
dataTest_ground_truth = np.load(f'batadal_gap10_test_ground_truth.npy', allow_pickle=False)
syn0=np.load(f'batadal_gap10_test_ground_truth.npy', allow_pickle=False)

syn1 = np.load(f'syntheticlabel.npy', allow_pickle=False)

syn2 = np.load(f'batadal_gap10_Test.npy', allow_pickle=False)
...
def linear(syn,index):
    ...
    X_test_r=dataTest_ground_truth[450:].copy()
