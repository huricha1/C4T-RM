...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('C4T-RM/BATADAL/batadal_gap10/'),
...
trainData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_gap10_Train.npy',
    allow_pickle=False)
testData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_gap10_Test.npy',
    allow_pickle=False)
valData = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_gap10_val.npy',
    allow_pickle=False)
test_ground_truth = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_gap10_test_ground_truth.npy',
    allow_pickle=False)
val_ground_truth = np.load(
    f'C4T-RM/BATADAL/batadal_gap10/batadal_gap10_val_ground_truth.npy',
    allow_pickle=False)
...
valData=processed_data[5160:6960].astype(np.float32).copy()
testData=processed_data[6960:].astype(np.float32).copy()
samples_list = list()
for i in range(0,trainData.shape[0]-opt.window_size+1,30):
...
class Generator_x(nn.Module):
    def __init__(self):
        ...
        self.time_series_layer = nn.Conv2d(
            in_channels=30, out_channels=30, kernel_size=(1, 3),stride=(1, 1), bias=True)
        # embedding
        self.encode = nn.Conv2d(
            in_channels=30, out_channels=30, kernel_size=(1, 40), stride=(1, 2), bias=True)
        self.encode1 = nn.Conv2d(
            in_channels=30, out_channels=30, kernel_size=(1, 25), stride=(1, 2), bias=True)

        self.lstm = nn.LSTM(input_size=36, hidden_size=100, num_layers=1, batch_first=True)
        # self.conv=nn.Conv1d(in_channels=36, out_channels=36, kernel_size=12, stride=4)
    ...
    def forward(self, x,up):
        ...
        time_series_emb = self.time_series_layer(x)

        channel_emb = []
        # channel embeddings

        channel_emb.append(self.channel_emb.unsqueeze(0).expand(len, -1, -1).unsqueeze(0).expand(batch_size,-1, -1, -1))
        # temporal embeddings
        tem_emb = []
        ...
        # concate embeddings
        hidden = torch.cat([time_series_emb] + channel_emb + tem_emb, dim=-1)

        out=self.encode(hidden)
        out=self.encode1(out)
        out,(h,c)=self.lstm(out.squeeze(-1))
        out=self.main(out)
        return out
