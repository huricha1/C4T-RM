...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('C4T-RM/IOT/iot_gap10/'),
...
trainData = np.load(f'C4T-RM/IOT/iot_gap10/iot_gap10_Train.npy',
                    allow_pickle=False)
testData = np.load(f'C4T-RM/IOT/iot_gap10/iot_gap10_Test.npy',
                   allow_pickle=False)
valData = np.load(f'C4T-RM/IOT/iot_gap10/iot_gap10_val.npy',
                  allow_pickle=False)
test_ground_truth = np.load(
    f'C4T-RM/IOT/iot_gap10/iot_gap10_test_ground_truth.npy',
    allow_pickle=False)
val_ground_truth = np.load(
    f'C4T-RM/IOT/iot_gap10/iot_gap10_val_ground_truth.npy',
    allow_pickle=False)
...
valData=processed_data[2610:3510].astype(np.float32).copy()
testData=processed_data[3510:].astype(np.float32).copy()
samples_list = list()
...
class Generator_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm0 = nn.LSTM(input_size=6, hidden_size=32, num_layers=1, batch_first=True)
...
class Discriminator_i(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.LSTM(input_size=6, hidden_size=64, num_layers=2, batch_first=True)
        )
